...
/// Counts the attempts a handler has served, so it can answer a retry differently.
...
struct LRCLibProviderTests {
    ...
    ) -> (LRCLibProvider, StubURLProtocol.Stub) {
        provider(retryPolicy: retryPolicy, sleep: sleep, outcomes: { request in
    ...
    private func provider(
        ...
        let provider = LRCLibProvider(
            session: stub.session, userAgent: "Lyribar/test (unit)",
    ...
    @Test func getHitReturnsSyncedLyrics() async throws {
        let (provider, stub) = provider { _ in
    ...
    @Test func serverErrorOnGetThrows() async throws {
        let (provider, _) = provider { _ in (500, self.body("oops")) }
    ...
    @Test func serverErrorIsRetriedThenSucceeds() async throws {
        ...
        let (provider, stub) = provider(retryPolicy: .default, sleep: sleep) { _ in
    ...
    @Test func transportErrorIsRetriedThenSucceeds() async throws {
        let sleep = SleepRecorder()
        let attempts = Attempts()
        let (provider, stub) = provider(retryPolicy: .default, sleep: sleep, outcomes: { _ in
            if attempts.next() == 1 { return .failure(URLError(.timedOut)) }
    ...
    @Test func persistentTransportErrorGivesUpAfterBoundedRetries() async throws {
        let (provider, stub) = provider(retryPolicy: .default, outcomes: { _ in
    ...
    @Test func requestCarriesUserAgent() async throws {
        let stub = StubURLProtocol.stub { _ in (200, self.body(#"{"syncedLyrics":"[00:01.00]Hi"}"#)) }
        let provider = LRCLibProvider(
            session: stub.session, userAgent: "Lyribar/1.2.3 (https://example.com)")

        _ = try await provider.fetch(track)

        #expect(
            stub.requests.first?.value(forHTTPHeaderField: "User-Agent")
                == "Lyribar/1.2.3 (https://example.com)")
    }
